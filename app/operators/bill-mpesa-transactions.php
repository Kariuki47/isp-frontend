 ...
 */
    ...
    $sqlfields = (array_key_exists('sqlfields', $_GET) && !empty($_GET['sqlfields']) && is_array($_GET['sqlfields']) &&
                  array_intersect($_GET['sqlfields'], array_keys($bill_mpesa_transactions_options_all)) == $_GET['sqlfields'])
               ? $_GET['sqlfields'] : $bill_mpesa_transactions_options_default;
    
    $cols = array();
    foreach ($sqlfields as $sqlfield) {
        $cols[$sqlfield] = $bill_mpesa_transactions_options_all[$sqlfield];
    }
    $colspan = count($cols);
    $half_colspan = intval($colspan / 2);
    
    $orderBy = (array_key_exists('orderBy', $_GET) && isset($_GET['orderBy']) &&
                in_array($_GET['orderBy'], array_keys($bill_mpesa_transactions_options_all)))
             ? $_GET['orderBy'] : array_keys($bill_mpesa_transactions_options_all)[0];
    ...
    $txn_type = (array_key_exists('txn_type', $_GET) && isset($_GET['txn_type']) &&
                    in_array($_GET['txn_type'], array_slice($valid_txnTypes, 1))) // avoid inserting "Any" in the SQL query
                 ? $_GET['txn_type'] : "";

    $payer_phone = (array_key_exists('payer_phone', $_GET) && !empty(str_replace("%", "", trim($_GET['payer_phone']))))
                 ? str_replace("%", "", trim($_GET['payer_phone'])) : "";
    ...
    userBillingMpesaSummary($startdate, $enddate, $payer_phone, $payment_status, $txn_type, 1);
    ...
    if (!empty($payer_phone)) {
        $sql_WHERE[] = sprintf("payer_phone LIKE '%s%%'", $dbSocket->escapeSimple($payer_phone));
        $partial_query_string_pieces[] = sprintf("payer_phone=%s", $payer_phone);
    }
    ...
    if (!empty($txn_type)) {
        $sql_WHERE[] = sprintf("txn_type='%s'", $dbSocket->escapeSimple($txn_type));
        $partial_query_string_pieces[] = sprintf("txn_type=%s", $txn_type);
    }
    
    // executing the custom query

    $sql = sprintf("SELECT %s FROM %s", implode(", ", $sqlfields), 'billing_mpesa');
